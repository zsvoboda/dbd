...
def validate_result(engine, table_name='test_typed', compensation=0, ):
    with engine.connect() as conn:
        ...
        assert result.fetchall() == [(3 - compensation,)], f"test_datetime IS NULL row count failed for table {table_name}"
        ...
        assert result.fetchall() == [(3 - compensation,)], f"test_string IS NULL row count failed for table {table_name}"
